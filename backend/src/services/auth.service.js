...
const { generateAccessToken, generateVerificationToken, hashToken } = require('../utils/jwt');

class AuthService {
  async register(userData, req) {
    const existingUser = await UserRepository.findByEmail(userData.email);
    if (existingUser) {
      throw new ApiError(httpStatus.CONFLICT, 'Email already in use', 'EMAIL_EXISTS');
    }
    ...
    await AuditRepository.saveVerificationToken(newUser.id, tokenHash, 'EMAIL_VERIFICATION', expiresAt);
    ...
    EmailService.sendVerification(newUser, verificationUrl).catch(console.error);

    AuditRepository.logAction(newUser.id, 'REGISTER', { email: newUser.email }, req.ip);
  ...
  async login(email, password, req) {
    ...
    if (!user) {
      throw new ApiError(httpStatus.UNAUTHORIZED, 'Invalid credentials', 'INVALID_CREDENTIALS');
    }

    if (user.lockout_until && new Date() < new Date(user.lockout_until)) {
      throw new ApiError(httpStatus.FORBIDDEN, 'Account is temporarily locked due to too many failed attempts.', 'ACCOUNT_LOCKED');
    }
    ...
    if (!isMatch) {
      ...
      if (attempts >= 5) {
        ...
        lockoutUntil = new Date(Date.now() + 15 * 60 * 1000).toISOString().slice(0, 19).replace('T', ' ');
        AuditRepository.logAction(user.id, 'ACCOUNT_LOCKED', { reason: 'Too many failed logins' }, req.ip);
      }
    ...
    if (!user.is_active) {
      throw new ApiError(httpStatus.FORBIDDEN, 'Account is disabled', 'ACCOUNT_DISABLED');
    }
    ...
    const { sessionId, plainRefreshToken } = await SessionService.createSessionAndRefreshToken(user.id, req);
  ...
  async verifyEmail(plainToken) {
    const tokenHash = hashToken(plainToken);
    const dbToken = await AuditRepository.findVerificationToken(tokenHash, 'EMAIL_VERIFICATION');

    if (!dbToken) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Invalid or expired verification token.', 'INVALID_TOKEN');
    }
