...
class PasswordService {
  async forgotPassword(email, req) {
    ...
    await AuditRepository.saveVerificationToken(user.id, tokenHash, 'PASSWORD_RESET', expiresAt);
  ...
  async resetPassword(plainToken, newPassword, req) {
    const tokenHash = hashToken(plainToken);
    const dbToken = await AuditRepository.findVerificationToken(tokenHash, 'PASSWORD_RESET');

    if (!dbToken) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Invalid or expired reset token.', 'INVALID_TOKEN');
    }
  ...
  async changePassword(userId, oldPassword, newPassword, req) {
    ...
    if (!isMatch) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Incorrect old password', 'INCORRECT_PASSWORD');
    }
