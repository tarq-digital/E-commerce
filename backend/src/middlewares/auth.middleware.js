...
const requireAuth = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (decoded.sessionId) {
      ...
      SessionRepository.updateLastActivity(decoded.sessionId).catch(console.error);
    }
...
const requirePermission = (permissionName) => {
  return catchAsync(async (req, res, next) => {
    ...
    const permissions = await PermissionRepository.findByRoleId(req.user.role_id);
    if (!permissions.includes(permissionName)) {
      throw new ApiError(httpStatus.FORBIDDEN, `Requires permission: ${permissionName}`, 'FORBIDDEN_ACCESS');
    }
