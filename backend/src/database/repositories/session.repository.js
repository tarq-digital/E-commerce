...
class SessionRepository {
  async createSession(sessionData) {
    const { user_id, device_id, user_agent, ip_address, expires_at } = sessionData;
  ...
  async revokeSession(sessionId) {
    await pool.query('UPDATE sessions SET revoked_at = CURRENT_TIMESTAMP WHERE id = ?', [sessionId]);
    // Cascade delete associated refresh tokens to guarantee invalidation
    await pool.query('DELETE FROM refresh_tokens WHERE session_id = ?', [sessionId]);
  }

  async revokeAllUserSessions(userId) {
    await pool.query('UPDATE sessions SET revoked_at = CURRENT_TIMESTAMP WHERE user_id = ?', [userId]);
