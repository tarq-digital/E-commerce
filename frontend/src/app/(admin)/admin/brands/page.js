...
export default function AdminBrandsPage() {
  ...
  const fetchBrands = async (search = '') => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000/api/v1'}/admin/brands?${query}`, {
  ...
  const columns = [
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full border border-gray-200 overflow-hidden relative shrink-0 bg-white">
            {row.logo_url ? (
              <Image src={row.logo_url} alt={row.name} fill className="object-contain p-1" sizes="40px" />
            ) : (
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage brand profiles and logos.</p>
        </div>
